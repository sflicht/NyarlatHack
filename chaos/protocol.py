...
EVENTS = frozenset('eat read zap apply pray kill level_enter level_leave sanity insight death sleep session safe_point ack telegraph expiry'.split())
REASONS = frozenset('ok schema oversize duplicate schedule budget active ineligible log_failure'.split())
NUMBERS = ('v', 'seq', 'turn', 'safe', 'sanity', 'insight', 'budget', 'spent', 'reserved', 'last_id')
...
def parse_request(raw):
    ...
    for k in FIELDS:
        if k != 'mutation': integer(r[k], 1 if k in ('id', 'at') else 0)
    name = r['mutation']
    ...
    if name == 'ambient':
        ...
        valid = r['value'] == (50 if name == 'ward_efficacy' else 2) and 1 <= r['duration'] <= 50
    if not valid:
...
def parse_event(raw):
    ...
    if e['v'] != 1 or e['event'] not in EVENTS or e.get('phase') not in ('attempt', 'result'):
    ...
    if e['event'] == 'ack':
        for k in ('id', 'value', 'duration', 'telegraph', 'at', 'cost', 'expires'):
            integer(e.get(k))
        if e.get('status') not in ('accepted', 'rejected') or e['detail'] not in REASONS:
