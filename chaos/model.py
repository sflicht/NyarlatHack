    ...
    def __init__(
        ...
        *,
        ...
        local = (
            allow_local_http
            and u.scheme == "http"
            and u.hostname in ("127.0.0.1", "::1", "localhost")
        )
        if (
            ...
            raise ValueError(
                "explicit HTTPS endpoint required (local HTTP fixture opt-in only)"
            )
        ...
        if (
            ...
            or not 2048 <= max_context <= 32768
            or not 512 <= max_response <= 65536
        ):
    ...
    def choose(self, state, ident, at):
        ...
        try:
            ...
            if response.status != 200:
                raise ValueError(
                    "model HTTP response rejected; no redirects or retries"
                )
            chunks = []
            size = 0
            while True:
                remaining = end - time.monotonic()
                if remaining <= 0:
                    raise ValueError("model request deadline exceeded")
                ...
                chunks.append(chunk)
                size += len(chunk)
                if size > self.max_response:
                    raise ValueError("model response byte cap exceeded")
