                         ...
                         size_t *canonical_length)
{
    ...
    return chaos_next_use_admit(out, &source, &gate, &envelope, canonical,
                                *canonical_length, 0, at_move, 1, receipt_ok,
                                NULL);
}
...
{
    int at_safe, at_move, origin_root, ttl, probe;
    const char *tamper = "";
    if (argc < 6) return 2;
    at_safe = atoi(argv[1]);
    at_move = atoi(argv[2]);
    origin_root = atoi(argv[3]);
    ttl = atoi(argv[4]);
    probe = atoi(argv[5]);
    if (argc > 6) tamper = argv[6];
    return run_install(at_safe, at_move, origin_root, ttl, probe, tamper);
}
