...
def validate_fatal(records, native, terminal, xlog, public, *, enabled):
    ...
    if enabled:
        root = observations[-1]
        assert root['observation'] == dict(operation='fountain_drink', stage='started', root_seq=0, fact='none')
        ...
        assert all(v == dict(count=0, saturated=False) for k, v in public['coverage'].items() if k != 'incomplete')
...
def reject_mutated_death_evidence(records, native, terminal, xlog, public):
    ...
    variants = {
        ...
        'no_native_gameover': (records, dict(native, gameover=0), terminal, xlog, public),
...
class FatalOracleTests(unittest.TestCase):
    def test_missing_native_death_rejected_before_journal(self):
        with self.assertRaises(AssertionError):
            validate_fatal([], dict(gameover=0, hp=1, entered=1, returned=0), b'', b'', {}, enabled=True)

    def test_pair_rejects_each_artifact_change(self):
        row = dict(terminal=b'a', inputs=['79'], xlog=b'b', dump=b'c', native={'hp': -1}, before={'hp': 1}, records=[])
