...
from test_episode_fountain_fatal_oracle import (
    compare_pair,
    validate_fatal,
    reject_mutated_death_evidence,
)
from test_episode_platforms import (
    Cancellation,
    bounded,
    digest,
    observe,
...
def main(argv=None):
    ...
    env = dict(
        selection.environment, HOME=str(out), TMPDIR=str(out), MAIL=str(out / "MAIL")
    )
    ...
    cancel = Cancellation()
    handlers = {
        sig: signal.signal(sig, cancel.handler)
    ...
    try:
        ...
        assert (
            chosen["fate"] == 21 and 1 <= chosen["seed"] == chosen["candidates"] <= 4096
        )
        ...
        for case in ("healthy", "fatal"):
            ...
            for enabled in (False, True):
                ...
                child_env = dict(
                    env,
                    FATAL_FOUNTAIN_SEED=str(chosen["seed"]),
                    FATAL_FOUNTAIN_CASE=case,
                )
        ...
        save(
            ...
            dict(
                status="PASS",
                scope="original main/moveloop with test-only first-dodrink baseline; not ordinary-play setup or complete turnloop acceptance",
                results=all_results,
            ),
        )
