    ...
    return event(
        ...
        **dict(
            dict(
                ...
                spent=1,
                budget=1,
                **{k: v for k, v in (request or REQ).items() if k != "v"},
            ),
            **kw,
        ),
    )
...
class DirectorTests(unittest.TestCase):
    def setUp(self):
        self.assertIsNotNone(
            importlib.util.find_spec("chaos"), "director package missing"
        )
    ...
    def test_schema_strict(self):
        ...
        for key in REQ:
            ...
            with self.assertRaises(ValueError):
                self.p.parse_request(json.dumps(r).encode())
        bad = [
            dict(REQ, id=True),
            dict(REQ, id=0),
            dict(REQ, at=0),
            dict(REQ, id=2147483648),
            dict(REQ, value=4),
            dict(REQ, duration=1),
            dict(REQ, telegraph=0),
            dict(REQ, cost=0),
            dict(REQ, mutation="shell"),
            dict(REQ, value=1.0),
        ]
    ...
    def test_reader_rejects_invalid_and_overlong(self):
        for raw in [
            ...
            b'{"seq":true}\n',
            b"x" * 4097,
            json.dumps(event(seq=True)).encode() + b"\n",
            ...
            with self.subTest(raw=raw[:50]), self.assertRaises(ValueError):
                self.d.EventReader(path).read()

    def test_reader_rewrite_truncation_and_caps(self):
        path = self.path / "events.jsonl"
        append(path, event())
        r = self.d.EventReader(path)
        r.read()
        path.write_bytes(b"")
        with self.assertRaises(ValueError):
            r.read()
        ...
        with self.assertRaises(ValueError):
            r.read()
        with self.assertRaises(ValueError):
            self.d.EventReader(path, max_bytes=10).read()
        append(path, event(2))
        with self.assertRaises(ValueError):
            self.d.EventReader(path, max_events=1).read()

    def test_summary_redaction_and_rollback(self):
        s = self.d.State()
        s.ingest(event(hidden_map="SECRET", detail="SECRET\u001b[31m"))
        self.assertNotIn("SECRET", s.summary())
        self.assertNotIn("hidden_map", s.summary())
        with self.assertRaises(ValueError):
            s.ingest(event())
        with self.assertRaises(ValueError):
            s.ingest(event(2, safe=0))
        for i in range(2, 102):
            s.ingest(event(i))
        self.assertLess(len(s.summary()), 4096)
        s.ingest(event(102, event="death", detail="quit"))
        self.assertTrue(s.ended)

    def test_budgets_and_seeded_eligibility(self):
        s = self.d.State()
        s.ingest(event())
        a = self.d.RandomBackend(42)
        b = self.d.RandomBackend(42)
        self.assertEqual(a.choose(s, 1, 2), b.choose(s, 1, 2))
        self.assertEqual(a.choose(s, 1, 2)["mutation"], "ambient")
        s.ingest(event(2, budget=0, spent=2))
        self.assertIsNone(a.choose(s, 2, 2))
        s = self.d.State()
        s.ingest(event(sanity=80, budget=4))
        self.assertEqual(
            set(self.d.eligible(s, ordinary_food=True)),
            {"ambient", "ward_efficacy", "hunger_rate"},
        )
        s.ingest(
            ack(
                request=dict(
                    REQ, mutation="ward_efficacy", value=50, duration=5, telegraph=2
                ),
                sanity=80,
                budget=0,
                spent=4,
    ...
    def test_atomic_mailbox_resume_no_clobber(self):
        ...
        target.unlink()
        target.symlink_to(self.path / "victim")
        with self.d.Mailbox(self.path) as resumed:
