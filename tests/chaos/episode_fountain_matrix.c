...
static int special;
static const char *special_names[] = { "ordinary", "magic-refresh",
    "magic-low-luck", "magic-high-luck", "magic-negative-luck",
    "depletion", "hallucination-map", "no-mouth" };
...
static void context_record(FILE *f) {
    fprintf(f,"{\"turn\":%ld,\"safe\":%ld,\"sanity\":%d,\"insight\":%d,"
            ...
            u.chaos.reserved,u.chaos.last_id,Upolyd?u.mh:u.uhp,Upolyd?u.mhmax:u.uhpmax,u.uen,u.uenmax);
}
static void snapshot(FILE *f) {
    ...
    fputc(']',f);
    if(special) {
        fprintf(f,",\"vision\":{\"recalc\":%d,\"rows\":[",vision_full_recalc);
        for(y=0;y<ROWNO;y++){if(y)fputc(',',f);hex(f,viz_array[y],COLNO);}
        fputs("]}",f);
    }
    fputc('}',f);
}
int main(int argc,char **argv) {
    ...
    if(!strcmp(argv[1],"--calibrate-specials")) {
        puts("[");
        for(seed=1;seed<=4096;seed++) {
            int dry=-1;
            seed_reset(seed);fate=rnd(30);
            /* Exact tasteless native branch: initial fate then dryup draw. */
            if(fate==10)dry=rn2(3);
            printf("%s{\"seed\":%u,\"fate\":%d,\"dry\":%d,\"draws\":%d}",seed==1?"":",",seed,fate,dry,reseed_count);
        }
        puts("]");return 0;
    }
    if(getenv("FOUNTAIN_CASE")) {
        for(special=1;special<=7;special++)
            if(!strcmp(getenv("FOUNTAIN_CASE"),special_names[special]))break;
        assert(special<=7);
    }
    ...
    if(special) {
        urace=races[0];assert(urace.malenum==PM_HUMAN);
        if(special==2||special==3)for(i=0;i<A_MAX;i++)ABASE(i)=8;
        u.uluck=special==3?4:special==4?-1:0;
        if(special==7) {
            u.umonnum=PM_NIGHTGAUNT;youmonst.data=&mons[PM_NIGHTGAUNT];
            youmonst.mtyp=PM_NIGHTGAUNT;u.mh=u.mhmax=100;
            assert(nomouth(youracedata->mtyp));
        }
    }
    ...
    if(special) {
        levl[10][10].blessedftn=(special<=4);
        assert(!wizard&&!in_town(10,10));
    }
    vision_init();vision_reset();vision_recalc(0);
    if(special==6){make_hallucinated(100L,FALSE,0L);assert(Hallucination);}
    ...
    assert(result==(special==7?MOVE_INSTANT:MOVE_QUAFFED)&&reseed_period==INT_MAX);
    ...
    if(special==1)assert(count==2&&levl[10][10].blessedftn&&level.flags.nfountains==1);
    if(special==2||special==3) {
        int gained=0;
        for(i=0;i<A_MAX;i++){assert(ABASE(i)>=12&&ABASE(i)==AMAX(i));gained+=ABASE(i)-12;}
        assert(gained==(special==2?1:A_MAX));
        assert(!levl[10][10].blessedftn&&levl[10][10].typ==FOUNTAIN&&level.flags.nfountains==1);
    }
    if(special==4)assert(u.uluck==-1&&u.uhunger<hunger&&multi==-2);
    if(special==5)assert(levl[10][10].typ==ROOM&&level.flags.nfountains==0&&count==2);
    if(special==6)assert(Hallucination&&fmon&&fmon->mtyp==PM_LITTLE_DOG);
    if(special==7)assert(count==0&&u.uhunger==hunger&&levl[10][10].typ==FOUNTAIN);
    ...
    if(special) {
        fprintf(f,",\"special\":{\"case\":\"%s\",\"blessed\":%d,\"luck\":%d,\"hallucination\":%ld,\"fountains\":%d,\"attributes\":[",special_names[special],levl[10][10].blessedftn,u.uluck,(long)HHallucination,level.flags.nfountains);
        for(i=0;i<A_MAX;i++){if(i)fputc(',',f);fprintf(f,"[%d,%d]",ABASE(i),AMAX(i));}
        fputs("]}",f);
    }
