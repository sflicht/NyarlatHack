...
class ProtocolTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sources = [
            ROOT / "src/chaos_protocol.c",
            ROOT / "tests/chaos/protocol_harness.c",
        ]
    ...
    def run_c(self, payload, mode="parse"):
        self.assertTrue(
            self.exe.exists(), "bounded C protocol implementation is missing"
        )
        return (
            subprocess.check_output([str(self.exe), mode], input=payload)
            .decode()
            .strip()
        )
        ...
        for key in ["v", "id", "value", "duration", "telegraph", "at"]:
            for value in [True, False, 1.0, 1e100, -1, 2147483648, "1", None, [], {}]:
                with self.subTest(key=key, value=value):
                    self.check_request(dict(REQUEST, **{key: value}), "schema")
        for token in ["01", "+1", "1e0", "-0"]:
            self.assertEqual(
                self.run_c(
                    json.dumps(REQUEST).replace('"id": 1', '"id": ' + token).encode()
                ),
                "schema",
            )
    ...
    def test_malformed_and_size(self):
        ...
        for raw in [
            ...
            valid + b"x",
            valid + b"\x00",
            valid.replace(b"ambient", b"ambi\nent"),
            valid.replace(b"ambient", b"ambi\x00ent"),
            valid.replace(b"ambient", b"ambi\\u0065nt"),
            valid.replace(b"ambient", b"ambi\x7fent"),
            valid[:-1] + b",}",
            valid[:-1],
            b"{" * 100,
        ]:
            with self.subTest(raw=raw):
                self.assertEqual(self.run_c(raw), "schema")
