...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.end_headers()
        try: self.wfile.write(self.server.body)
        except (BrokenPipeError,ConnectionResetError): pass

class ModelTests(unittest.TestCase):
    def setUp(self):
        self.assertIsNotNone(importlib.util.find_spec('chaos.model'),'model backend missing')
        from chaos.model import ModelBackend
        self.ModelBackend=ModelBackend
        self.server=ThreadingHTTPServer(('127.0.0.1',0),Handler)
        self.server.requests=[]; self.server.code=200; self.server.delay=0
        self.server.redirect='http://127.0.0.1:1/stolen'
        self.server.body=json.dumps({'choices':[{'message':{'content':json.dumps(dict(REQ,at=2))}}]}).encode()
        ...
        self.state=State(); self.state.ingest(event(detail='SECRET', hidden='SECRET'))

    def backend(self,**kw):
        return self.ModelBackend(self.url,'local-fake-fixture','CHAOS_LOCAL_FIXTURE_KEY',
                                 **dict(dict(allow_local_http=True,timeout=.2),**kw))
    ...
    def test_malformed_duplicate_extra_and_schedule_rejected(self):
        for content in ['not json',json.dumps(REQ),json.dumps(dict(REQ,at=2,id=True)),
                        json.dumps(dict(REQ,at=2,command='rm')),json.dumps(dict(REQ,at=2))[:-1]+',"id":1}']:
            self.server.body=json.dumps({'choices':[{'message':{'content':content}}]}).encode()
            with self.subTest(content=content), self.assertRaises(ValueError): self.backend().choose(self.state,1,2)
    ...
    def test_no_budget_no_requests_and_no_event_hammer(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            run(tmp,self.backend(),max_runtime=.06,poll=.01)
    ...
    def test_timeout_no_retry(self):
        ...
        with self.assertRaises((ValueError,OSError)): self.backend(timeout=.03).choose(self.state,1,2)
        self.assertLess(time.monotonic()-start,.3)
        self.assertEqual(len(self.server.requests),1)

    def test_redirects_and_oversize_rejected(self):
        self.server.code=302
        with self.assertRaises(ValueError): self.backend().choose(self.state,1,2)
        self.assertEqual(len(self.server.requests),1)
        self.server.code=200; self.server.body=b'x'*16385
        with self.assertRaises(ValueError): self.backend().choose(self.state,1,2)
        self.assertEqual(len(self.server.requests),2)

    def test_explicit_endpoint_env_and_limits(self):
        for kwargs in [dict(allow_local_http=False),dict(max_calls=0),dict(timeout=0),dict(max_context=10),dict(max_response=0)]:
            with self.subTest(kwargs=kwargs), self.assertRaises(ValueError): self.backend(**kwargs)
        for url in ['http://example.com/api','https://user:secret@example.com/api','https://example.com/api#fragment']:
            with self.subTest(url=url), self.assertRaises(ValueError):
                self.ModelBackend(url,'fixture','CHAOS_LOCAL_FIXTURE_KEY',allow_local_http=True)
        self.assertEqual(len(self.server.requests),0)
