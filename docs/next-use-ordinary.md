...
`play --next-use` publishes one envelope when a ready origin schedule matches the public menu. The choice is `RandomHistoryBackend` seed 0, so it may be quiet or `whistle_attention`. It sets observations and admission on the game process only. It does not call a model. Default `play` does not do this.
