...
Message notices require confirmed delivery through the native window port, not merely execution of `pline` or arrival at `vpline`'s `putstr(WIN_MESSAGE,...)` call. NOSHOW/NOREP can suppress messages earlier; the TTY window port can also suppress rendering under WIN_STOP. Before adding production hooks, test those suppression paths and record only a supported port's actual delivery, failing closed for unverified delivery paths. Arm only a fixed enum around the chosen native message call. At entry to that `vpline`, take an action-bound token into a local (so nested messages cannot steal it). The token contains the originating root sequence and fixed fact enum; delivery must match both against the current scope. Emit only after verified delivery, or discard the token on an early return. Map delivery must likewise take and validate an action-bound token. A previously taken token must not become valid for a replacement action merely because both actions use the same fact. Disarm immediately after the native call. Never pass message strings into observation code. Scope map delivery similarly around fountain's existing `monster_detect(NULL,0)`; record after its actual `display_nhwindow(WIN_MAP,TRUE)`, never from fate or a second detection call.
