...
/* Positive pending facts are armed; negative ones were taken by vpline.
 * This keeps a delivery token without another persistent or transient field. */
...
int chaos_observation_take_message(void) {
    ...
    if (!observation_current() || observation_used || observation_pending <= 0
        || observation_pending == CHAOS_OBS_FACT_DETECTION_PRESENTED) return 0;
    fact = observation_pending;
    observation_pending = -fact;
    return fact;
}
...
void chaos_observation_delivered(int fact) {
    if (!observation_current() || observation_used
        || fact < CHAOS_OBS_FACT_SOUND_HIGH || fact > CHAOS_OBS_FACT_CANNOT_REACH
        || observation_pending != -fact) return;
    observation_notice(fact);
}
void chaos_observation_map_delivered(void) {
    if (!observation_current() || observation_used
        || observation_operation != CHAOS_OBS_OP_FOUNTAIN_DRINK
        || observation_pending != CHAOS_OBS_FACT_DETECTION_PRESENTED) return;
    observation_notice(CHAOS_OBS_FACT_DETECTION_PRESENTED);
}
